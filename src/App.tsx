...
import { api, ApiError, Message, Post, User } from './api';
...
export default function App(){const [section,setSection]=useState<Section>('home');const[user,setUser]=useState<User|null>(null);const[messageTarget,setMessageTarget]=useState<User|null>(null);const[loading,setLoading]=useState(true);useEffect(()=>{api.session().then(r=>setUser(r.user)).catch(()=>setUser(null)).finally(()=>setLoading(false));},[]);if(loading)return <div className="app-shell"><div className="loading">Loading Sphere…</div></div>;if(!user)return <Auth onAuth={setUser}/>;const go=(s:Section)=>setSection(s);const openMessages=(u:User)=>{setMessageTarget(u);setSection('messages')};return <div className="app-shell"><header className="topbar"><button className="brand brand-button" onClick={()=>go('home')}>Sphere</button><div className="topbar-actions"><button aria-label="Notifications" onClick={()=>go('notifications')}>♡</button><button aria-label="Messages" onClick={()=>go('messages')}>✉</button></div></header><main className="content">{section==='home'&&<Home user={user}/>}{section==='search'&&<Search onMessage={openMessages}/>} {section==='create'&&<Create onCreated={()=>go('home')}/>} {section==='earn'&&<Earn/>}{section==='wallet'&&<Wallet/>}{section==='profile'&&<Profile user={user} onLogout={()=>api.logout().finally(()=>setUser(null))} onUpdated={setUser} onSettings={()=>go('settings')}/>} {section==='settings'&&<Settings/>}{section==='notifications'&&<Notifications/>}{section==='messages'&&<Messages targetUser={messageTarget}/>}</main><nav className="bottom-nav" aria-label="Primary navigation">{nav.map(([id,label,icon])=><button key={id} className={section===id?'active':''} onClick={()=>go(id)}><span className="nav-icon">{icon}</span><span>{label}</span></button>)}</nav></div>}
...
function Search({onMessage}:{onMessage:(u:User)=>void}){const[q,setQ]=useState('');const[users,setUsers]=useState<User[]>([]);const[posts,setPosts]=useState<Post[]>([]);const[busy,setBusy]=useState(false);async function run(){if(q.trim().length<2)return;setBusy(true);try{const r=await api.search(q);setUsers(r.users);setPosts(r.posts)}catch{}finally{setBusy(false)}}return <section><Heading eyebrow="Discover" title="Search"/><div className="search-row"><input value={q} onChange={e=>setQ(e.target.value)} onKeyDown={e=>e.key==='Enter'&&run()} placeholder="Search users or posts"/><button className="primary small" onClick={run} disabled={busy}>Search</button></div>{!users.length&&!posts.length?<Empty title={q?'No matches':'Find people and posts'} text="Search uses real Sphere data."/>:<div className="results">{users.map(u=><div className="result" key={u.id}><div className="avatar">{u.username[0]?.toUpperCase()}</div><div><strong>@{u.username}</strong><p className="muted">Sphere user</p></div><button className="quiet-button" onClick={()=>onMessage(u)}>Message</button></div>)}{posts.map(p=><div className="result" key={p.id||p.post_id}><strong>{p.caption||'Image post'}</strong><p className="muted">@{p.author?.username||'creator'}</p></div>)}</div>}</section>}
