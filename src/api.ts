...
export const api={
  ...
  login:async(i:{email?:string;username?:string;password:string})=>{const identifier=i.email||i.username||'';const r=await request<{user:User}>("/auth/login",{method:'POST',body:JSON.stringify({email:identifier,username:identifier,password:i.password})});return {user:r.user}},
  logout:async()=>request<{ok:true}>("/auth/logout",{method:'POST'}),
  ...
  me:()=>request<{user:User}>("/auth/session").then(r=>({user:r.user})),
  updateProfile:async(input:Partial<Pick<User,'username'|'bio'|'avatar_url'>>)=>{const body:any={};if(input.username!==undefined)body.username=input.username;if(input.bio!==undefined)body.bio=input.bio;if(input.avatar_url!==undefined)body.avatar_url=input.avatar_url;const r=await request<{user:User}>("/profile",{method:'PATCH',body:JSON.stringify(body)});return {user:r.user}},
  profile:(username:string)=>request<{profile:Profile}>(`/users/${encodeURIComponent(username)}`).then(r=>r),
  posts:async(limit=20,offset=0,feed:'forYou'|'following'='forYou')=>{const r=await request<{posts:Post[];page:number;hasMore:boolean}>(`/posts?limit=${limit}&page=${Math.floor(offset/Math.max(limit,1))+1}&feed=${encodeURIComponent(feed)}`);return {...r,posts:r.posts.map(normalizePost)}},
  like:async(id:string)=>{const r=await request<{liked:boolean;likesCount:number}>(`/posts/${encodeURIComponent(id)}/like`,{method:'POST'});return {liked:r.liked,saved:false,likesCount:r.likesCount}},
  save:async(id:string)=>{const r=await request<{saved:boolean}>(`/posts/${encodeURIComponent(id)}/save`,{method:'POST'});return {saved:r.saved,liked:false}},
  ...
  follow:async(user_id:string)=>{const r=await request<{following:boolean;pending?:boolean}>(`/follows`,{method:'POST',body:JSON.stringify({user_id})});return {following:r.following,pending:Boolean(r.pending)}},
  notifications:async()=>{const r=await request<{notifications:NotificationItem[];unreadCount?:number}>("/notifications");return {notifications:r.notifications,unreadCount:r.unreadCount||0}},
  ...
  updateSettings:async(settings:Record<string,unknown>)=>{const r=await request<{ok:true}>("/settings",{method:'PATCH',body:JSON.stringify(settings)});return {ok:true as const,settings}},
  ...
  uploadImage:async(file:File)=>{const f=new FormData();f.append('file',file);const r=await request<{url:string;key:string}>("/upload",{method:'POST',body:f});return r},
  createPost:async(i:{caption:string;music?:Music;imageKey:string})=>{const imageUrl=i.imageKey.startsWith('http')?i.imageKey:`${API_BASE}/api/storage/${encodeURIComponent(i.imageKey)}`;const r=await request<{post:Post}>("/posts",{method:'POST',body:JSON.stringify({caption:i.caption,imageUrl,song:i.music})});return {post_id:r.post.id,post:normalizePost(r.post)}},
