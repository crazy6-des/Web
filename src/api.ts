...
async function raw<T>(path:string,init:RequestInit={}):Promise<T>{
  ...
  let r:Response;
  try { r=await fetch(`${API_BASE}${path}`,{...init,headers,credentials:'include'}); }
  catch { throw new ApiError(`Unable to reach Sphere API at ${API_BASE}`,0); }
...
export const api={
  ...
  repost:async(id:string)=>request<{reposted:boolean;post_id:string}>(`/posts/${encodeURIComponent(id)}/repost`,{method:'POST'}),
