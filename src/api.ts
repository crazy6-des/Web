...
export const api={
  ...
  me:()=>request<{data:{user:User}}>("/auth/me").then(r=>({user:r.data.user})),
  ...
  createPost:async(i:{caption:string;music?:Music;imageKey:string})=>{const imageUrl=i.imageKey.startsWith('http')?i.imageKey:`${API_BASE}/api/storage/${encodeURIComponent(i.imageKey)}`;const r=await request<{data:{post:Post}}>("/posts",{method:'POST',body:JSON.stringify({caption:i.caption,imageUrl,song:i.music})});return {post_id:r.data.post.id,post:normalizePost(r.data.post)}},
